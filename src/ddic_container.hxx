...
namespace ddic {
  ...
  public:
       ...
       */
      explicit container(container* parent = nullptr)
              :parent_{parent} { }
      ...
      {
          auto factory = std::make_unique<default_factory<Type, Policy>>();
          auto proxy = register_proxy{factory.get(), *this};
          named_factories_[typeid(Type).name()] = factory.get();
      ...
      {
          auto factory = std::make_unique<prototype_factory<Type, Policy>>(proto);
          auto proxy = register_proxy{factory.get(), *this};
          named_factories_[typeid(Type).name()] = factory.get();
          factories_.push_back(std::move(factory));
          return proxy;
      ...
      {
          auto factory = std::make_unique<functor_factory<container, Type, Policy>>(fn, *this);
          auto proxy = register_proxy{factory.get(), *this};
          named_factories_[typeid(Type).name()] = factory.get();
          factories_.push_back(std::move(factory));
          return proxy;
  ...
  private:
      container* parent_; ///< Pointer to the parent container.
      std::vector<std::unique_ptr<abstract_factory>> factories_;
      std::unordered_map<std::string, abstract_factory*> named_factories_; ///< All the registered factories.
